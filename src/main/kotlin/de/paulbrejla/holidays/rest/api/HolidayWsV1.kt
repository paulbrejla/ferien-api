...
interface HolidayWsV1 {
    ...
    @RequestMapping("/holidays", method = [RequestMethod.GET])
    fun getHolidays(): List<HolidayDto>

    @RequestMapping("/holidays/{state}", method = [RequestMethod.GET])
    fun getHolidaysForState(@PathVariable("state") state: State): List<HolidayDto>

    @RequestMapping("/holidays/{state}/{year}", method = [RequestMethod.GET])
